...
static void mouse_callback(GLFWwindow *window, int button, int action, int mods)
{
	...
	if(action == GLFW_PRESS) {
		...
		int scaleX = ((xMaxWorld - xMinWorld) / width);
		cout <<scaleX << endl;
		int scaleY = ((yMaxWorld - yMinWorld) /height);
		newPt[0] = (scaleX * posX) + xMinWorld;
		newPt[1] = (scaleY * posY) + yMinWorld;
...
static void init()
{
	...
	colorAttrID = glGetAttribLocation(progID, "color");
	...
	vector<float> posBuf;
	//tri 1
	posBuf.push_back(-0.5f); // x
	posBuf.push_back(-0.5f); // y
...
static void render()
{
	...
	//point to color
	glEnableVertexAttribArray(colorAttrID);
	glBindBuffer(GL_ARRAY_BUFFER, colBufID);
	glVertexAttribPointer(colorAttrID, 3, GL_FLOAT, GL_FALSE, 0, (void*)(sizeof(float) * 2));
	...
	glDisableVertexAttribArray(colorAttrID);
