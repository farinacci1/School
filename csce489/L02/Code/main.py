...
from pyramid import Image as im
from Poisson import Image as im2
...
def PyramidBlend(source, mask, target):
    blPyr = im.blend(source, mask, target)
    collapsed = im.collapse(blPyr)
    final = im.minmaxNormalize(collapsed)
    return final

# Poisson Blend
def PoissonBlend(source, mask, target, isMix):
    return im2.poisson(source,target,mask,isMix)
...
if __name__ == '__main__':
    ...
    inputDir = '../Images/'
    outputDir = '../Results/'
    #src = plt.imread("../Images/source_09.jpg")/255
     
    #mask = GetMask(src)
    #plt.imsave("../Images/mask_9.jpg",mask)
    
    # False for source gradient, true for mixing gradients
    isMix = False

    # Source offsets in target
    offsets = [[0, 0], [0, 0], [210, 10], [10, 28], [140, 80], [-40, 90], [60, 100], [20, 20], [-28, 88],[50,100]]
